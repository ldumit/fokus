...
<template>
  <div class="flex flex-col gap-4">
    <div v-if="!flags.hasAnyFlags" class="flex items-center gap-2 text-status-success text-sm">
      ...
      <span title="All sprint health indicators passed with no issues detected.">No flags this sprint</span>
    </div>
    ...
    <div v-if="flags.zombieTickets.length > 0">
      <div class="text-sm font-medium text-status-warning mb-2 flex items-center gap-2">
        ...
        <span
          ...
          title="Tickets that have appeared in 3 or more sprints without being completed."
        >
    ...
    <div v-if="flags.midSprintDisruption">
      <div class="text-sm font-medium text-status-warning mb-2 flex items-center gap-2">
        ...
        <span
          class="text-text-muted cursor-help"
          :title="`Story points and ticket count added to the sprint after the first ${settingsStore.settings.planningWindowDays} days.`"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-3.5 h-3.5">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clip-rule="evenodd" />
          </svg>
    ...
    <div v-if="flags.zeroSpDevelopers.length > 0">
      <div class="text-sm font-medium text-status-warning mb-2 flex items-center gap-2">
        ...
        <span
          class="text-text-muted cursor-help"
          title="Active developers who had assigned tickets but completed zero story points."
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-3.5 h-3.5">
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clip-rule="evenodd" />
          </svg>
