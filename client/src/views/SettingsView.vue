...
import { getBoards, getStatuses, getCycleTimeBoundaries, saveCycleTimeBoundaries, getExcludedStatuses, saveExcludedStatuses, saveXraySettings, testXrayConnection, syncXrayData } from '../api/settings'
...
async function saveXrayPanel() {
  ...
  try {
    // Pass null for secret when field is empty — backend preserves the existing secret
      ...
      <template v-else>
        ...
        <div class="flex gap-1 border-b border-border-default mb-6">
          ...
          <button
            ...
            :class="['px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors', activeSettingsTab === 'xray' ? 'border-accent-default text-accent-default' : 'border-transparent text-text-secondary hover:text-text-primary']"
            @click="activeSettingsTab = 'xray'"
          >Xray</button>
          <button
            v-if="authStore.isAdmin"
        ...
        <!-- Xray tab -->
        <template v-if="activeSettingsTab === 'xray' && authStore.isAdmin">
